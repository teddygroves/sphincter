...
model {
  ...
  if (likelihood){
    ...
    for (n in 1:N_train){
      y_std[ix_train[n]] ~ normal(eta_std[n], sigma_std);
    }
  }
}
generated quantities {
  ...
  real sigma = sigma_std * sd(y);
  {
    ...
    for (n in 1:N_test){
      yrep[n] = normal_rng(eta[n], sigma);
      llik[n] = normal_lpdf(y_std[n] | eta_std[n], sigma_std);
    }
  }
}
