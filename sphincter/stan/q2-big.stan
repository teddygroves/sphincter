...
#include custom_functions.stan
  tuple(vector, vector) get_eta(
    ...
    for (n in 1:N){
      out.1[n] = mu[1]
      + b_pressure[1, vessel_type[n], treatment[n], age[mouse[n]]] * pressure_std[n]
      + a_treatment[1, treatment[n]] 
      + a_vessel_type[1, vessel_type[n]]
      + b_age[1, vessel_type[n], treatment[n]] * (1-age[mouse[n]]);
      out.2[n] = mu[2]
      + b_pressure[2, vessel_type[n], treatment[n], age[mouse[n]]] * pressure_std[n]
      + a_treatment[2, treatment[n]] 
      + a_vessel_type[2, vessel_type[n]]
      + b_age[2, vessel_type[n], treatment[n]] * (1-age[mouse[n]]);
    }
    return out;
  }
}
...
parameters {
  ...
  array[2, N_vessel_type, N_treatment] real b_age_z;
}
transformed parameters {
  array[2, N_treatment] real a_treatment;
  array[2, N_vessel_type] real a_vessel_type;
  array[2, N_vessel_type, N_treatment] real b_age;
  array[2, N_vessel_type, N_treatment, N_age] real b_pressure;
  for (i in 1:2){
    ...
    for (v in 1:N_vessel_type){
      ...
      for (t in 1:N_treatment){
        b_age[i, v, t] = b_age_z[i, v, t] * tau_age[i];
        for (a in 1:N_age)
          b_pressure[i, v, t, a] = 
            mu_b_pressure[i, v] 
