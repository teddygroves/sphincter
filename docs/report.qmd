...
The pulsatility data consisted of 

## Missing data

[DESCRIBE]

Again we assumed that all absent measurements were missing at random. 

## Statistical model

We knew from XXX that the power harmonics should individually follow exponential
distributions [REFERENCE FOR THIS]. The sum of two different power harmonics
with exponential rate parameters $\lambda_1$ and $\lambda_2$ should follow the
hypoexponential distribution with the probability density function shown in equation \eqref{}
